...
public class CharacterSelectionScreen extends VBox {
    ...
    public CharacterSelectionScreen(Stage stage, StorageService storageService, Consumer<Character> onCharacterSelected) {
        ...
        for (String name : names) {
            ...
            HBox characterRow = new HBox(10, avatar, nameLabel);
            characterRow.setAlignment(Pos.CENTER_LEFT);
            characterRow.setStyle("-fx-padding: 5; -fx-background-color: #2e2e2e; -fx-border-radius: 6; -fx-background-radius: 6;");

            characterRow.setOnMouseEntered(e ->
                    characterRow.setStyle("-fx-padding: 5; -fx-background-color: #3e3e3e; -fx-border-radius: 6; -fx-background-radius: 6;")
            );
            characterRow.setOnMouseExited(e ->
                    characterRow.setStyle("-fx-padding: 5; -fx-background-color: #2e2e2e; -fx-border-radius: 6; -fx-background-radius: 6;")
            );

            characterRow.setOnMouseClicked(e -> onCharacterSelected.accept(character));
